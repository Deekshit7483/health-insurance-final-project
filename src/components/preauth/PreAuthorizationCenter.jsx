...
const PreAuthorizationCenter = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      console.log('[PreAuthCenter] Fetching data...');
      console.log('[PreAuthCenter] PayorAPI auth headers:', payorAPI.payorAuth);
      ...
      console.log('[PreAuthCenter] Preauth response:', preauthResponse);
      console.log('[PreAuthCenter] Policy response:', policyResponse);
      
      setPreauths(preauthResponse.results || []);
      setPolicies(policyResponse.policies || []);
      setError(''); // Clear any previous errors
    } catch (err) {
      console.error('[PreAuthCenter] Error fetching data:', err);
      console.error('[PreAuthCenter] Error message:', err.message);
      setError(`Failed to load pre-authorization data: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
